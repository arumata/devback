...
func TestInit_RemovesLegacyRepoTemplatesDir(t *testing.T) {
	t.Helper()
	...
	// Create a file at the old legacy path.
	legacyDir := filepath.Join(homeDir, ".local", "share", "devback", "templates", "repo")
	if err := os.MkdirAll(legacyDir, 0o750); err != nil {
		t.Fatalf("create legacy dir: %v", err)
	}
	legacyFile := filepath.Join(legacyDir, "devbackignore")
	...
	// Legacy path must be removed.
	if _, err := os.Stat(legacyDir); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected legacy repo templates dir to be removed, got %v", err)
	}

	// New path must exist with the template.
	newDir := filepath.Join(homeDir, ".local", "share", "devback", "repo-templates")
	if _, err := os.Stat(filepath.Join(newDir, "devbackignore")); err != nil {
		t.Fatalf("expected new repo templates dir to be created: %v", err)
	}
}
